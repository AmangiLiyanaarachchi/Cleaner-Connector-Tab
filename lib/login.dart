...
Map<String, dynamic> loginSiteData = {
  'id': '',
  'admin_name': '',
  'email': '',
  'accessToken': '',
  ...
  Future LoginData() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // if (
        //     response.data["message"] == "Cleaner Loging success") {
        //   SharedPreferences sharedPreferences =
        //       await SharedPreferences.getInstance();
        //   sharedPreferences.setBool(SplashScreenState.KEYLOGIN, true);
        //   await sharedPreferences.setString('Admin token', response.data['token']);

        //   await sharedPreferences.setString('id', loginSiteData['id']);
        //   await sharedPreferences.setString('userType', 'cleaner');
        //   await sharedPreferences.setString('email', loginSiteData['email']);
        //   await sharedPreferences.setBool('login', false);

        //   //get username
        //   await ApiServices.getLoggedUserDetails(
        //           loginSiteData['id'], loginSiteData['accessToken'])
        //       .then(
        //     (value) async {
        ...
        if (response.data["message"] == "Client loging success") {
          ...
          await sharedPreferences.setString(
              'Client siteId', response.data['id']);
          await sharedPreferences.setBool('login', false);

          setState(() {
            loginSiteData['email'] = emailEditingController.text;
            loginSiteData['password'] = passwordEditingController.text;
            loginSiteData['userType'] = 'client';
            loginSiteData['accessToken'] = response.data['token'];
            loginSiteData['id'] = response.data['id'];
            print('User ID ************* ${loginSiteData['id']}');
          });
         ...
         Navigator.push(context,
          ...
          setState(() {
            loginSiteData['email'] = emailEditingController.text;
            loginSiteData['password'] = passwordEditingController.text;
            loginSiteData['userType'] = 'super admin';
            loginSiteData['accessToken'] = response.data['token'];
            loginSiteData['id'] = response.data['id'];
            print('User ID ************* ${loginSiteData['id']}');
          });

          await sharedPreferences.setString('userId', loginSiteData['id']);
